...
这段时间由于忙毕业前前后后的事情，拖更了很久，表示非常抱歉，回归后的第一篇文章主要是看到了Akka最新文档中写的[What problems does the actor model solve?](https://doc.akka.io/docs/akka/current/scala/guide/actors-intro.html),阅读完后觉得还是蛮不错，能简洁清晰的阐述目前并发领域遇到的问题，并为何利用Actor模型可以解决这些问题，本文主要是利用自己的理解将这篇文章进行翻译，有不足之处还请指出。
...
![seq chart](/media/images/2017/07/seq-chart.png)

但上述图表并不能准确地表示实例在执行过程中的生命线。实际上，一个线程执行所有这些调用，并且变量的操作也在调用该方法的同一线程上。为刚才的序列图加上执行线程，看起来像这样：

![seq chart thread](/media/images/2017/07/seq-chart-thread.png)

但当在面对多线程的情况下，会发现此前的图越来越混乱和变得不清晰，现在我们模拟多个线程访问同一个示例：

![seq chart multi thread](/media/images/2017/07/seq-chart-multi-thread.png)
...
![object graph](/media/images/2017/07/object-graph.png)

然后，在多线程分布式环境中，实际上线程是通过方法调用遍历这个对象实例网络。因此，线程是方法调用驱动执行的：

![object graph snakes](/media/images/2017/07/object-graph-snakes.png)
...
![exception prop](/media/images/2017/07/exception-prop.png)
...
![actor graph](/media/images/2017/07/actor-graph.png)
...
![serialized timeline invariants](/media/images/2017/07/serialized-timeline-invariants.png)
...
![actor tree supervision](/media/images/2017/07/actor-tree-supervision.png)
